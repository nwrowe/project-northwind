...
const PLAYER_FORWARD_FRAME_PATHS := [
...
func _setup_player_forward_walk_test() -> void:
	...
	var walk_frames := _load_forward_walk_frames()
	if walk_frames.is_empty():
		story_label.text = "Forward-walk test scene loaded, but res://art/characters/walking0.png through walking2.png could not be loaded yet."
		return
...
func _load_forward_walk_frames() -> Array[Texture2D]:
	var textures: Array[Texture2D] = []
	for frame_path in PLAYER_FORWARD_FRAME_PATHS:
